...
export default function App() {
  ...
  const refresh = async () => {
    ...
    if (web3auth?.provider) {
      ...
      const smartAccount = await SmartAccountSponsor.New(
        signer,
  ...
  const mock = async () => {
    if (smartAccount) {
      ...
      const mint = {
        to: Config.erc20,
        data: uniFace.encodeFunctionData("sudoMint", [
      ...
      const approve = {
        to: Config.erc20,
        data: uniFace.encodeFunctionData("approve", [
          Config.platform,
          10000n * 10n ** 18n,
        ]),
      } as any;
      const stake = {
        to: Config.platform,
        data: uniFace.encodeFunctionData("stake", [
          Config.erc20,
          1n * 10n ** 18n,
        ]),
      } as any;
      ...
      const transactionDetails =
        await smartAccount.sendTransaction(partialUserOp);
      setPending(false);
  ...
  const title = () => {
    return <Title>Demo For Social-AA</Title>;
  };
