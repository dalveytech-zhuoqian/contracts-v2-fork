...
export class SmartAccountSponsor {
  ...
  static New = async (
    wallet: Signer,
    chainId: number,
    overrides?: SmartAccountSponsorOverrides,
  ) => {
    const bundler: IBundler = new Bundler({
      bundlerUrl: `https://bundler.biconomy.io/api/v2/${chainId}/${
        overrides?.bundlerKey ??
    ...
    const paymaster = new BiconomyPaymaster({
      paymasterUrl: `https://paymaster.biconomy.io/api/v1/${chainId}/${
        overrides?.paymasterKey ??
        "vLtCApGzx.a74fe509-e670-40fa-a071-cff3fe7aed51"
      }`,
    });
    ...
    const smartAccount = await new BiconomySmartAccount(
      smartAccountConfig,
    ).init();
    ...
    return new SmartAccountSponsor(
      smartAccount,
      owner,
      smartAccountAddress,
      overrides?.debug,
  ...
  buildUserOp = async (
    ...
    const partialUserOp = await this.smartAccount.buildUserOp(
      transactions,
      overrides,
    ...
    const paymasterRes = await (
      this.smartAccount.paymaster as IHybridPaymaster<SponsorUserOperationDto>
    ).getPaymasterAndData(partialUserOp, paymasterServiceData);

    partialUserOp.paymasterAndData = paymasterRes.paymasterAndData;
    if (
      paymasterRes.callGasLimit &&
      paymasterRes.verificationGasLimit &&
      paymasterRes.preVerificationGas
    ) {
