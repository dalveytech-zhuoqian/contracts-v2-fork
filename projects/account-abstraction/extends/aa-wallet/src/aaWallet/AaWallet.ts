...
const EXECUTE_BATCH = {
  v6: {
    sig: ethers
      .keccak256(
        ethers.toUtf8Bytes("executeBatch(address[],uint256[],bytes[])"),
      )
      .substring(0, 10),
  ...
  v5: {
    sig: ethers
      .keccak256(ethers.toUtf8Bytes("executeBatch(address[],bytes[])"))
      .substring(0, 10),
    type: ["address[]", "bytes[]"],
  },
};
...
export class AaWallet {
  ...
  protected constructor(
    ...
    aaAddress: string,
    chainId: bigint,
    opt = {} as AaWallet.Options,
  ) {
    this.address = aaAddress;
    this.version = opt.version ?? VERSION.v6;
    this.signer = signerWithProvider;
    this.user = user;
    this.salt = opt.salt ?? 0n;
    this.chainId = chainId;
    this.entryPoint = new Contract(
      opt.entryPoint ?? ERC4337.EntryPoint,
      EntryPointAbi,
      this.signer,
    ) as any as EntryPoint;
    this.aaAccount = new Contract(
      aaAddress,
      SimpleAccountAbi,
      this.signer,
    ) as any as SimpleAccount;
    this.aaAccountFactory = new Contract(
      opt.aaAccountFactory ?? ERC4337.SimpleAccount.Factory,
      SimpleAccountFactoryAbi,
      this.signer,
    ) as any as SimpleAccountFactory;
    this.creationCode = this.aaAccountFactory.interface.encodeFunctionData(
      "createAccount",
      [this.user, this.salt],
    );
    this.initCode = ethers.concat([
      this.aaAccountFactory.target as string,
      this.creationCode,
    ]);
  }
  ...
  public static connect = async (
    ...
    if (!signerWithProvider.provider)
    ...
    const aaAddress =
      "0x" +
      (
        await signerWithProvider.call({
          to: opt?.aaAccountFactory ?? ERC4337.SimpleAccount.Factory,
          data:
            ethers
              .keccak256(ethers.toUtf8Bytes("getAddress(address,uint256)"))
              .substring(0, 10) +
            AbiCoder.defaultAbiCoder()
              .encode(["address", "uint256"], [user, opt.salt ?? 0n])
              .substring(2),
          gasLimit: 3000000,
        })
      ).substring(26);
  ...
  public sign = async (
    action: Action[],
    ...middlewareFunc: Array<
      (op: UserOperation, chainId: bigint) => Promise<UserOperation>
    >
  ) => {
    ...
    let userOp = {
      ...
      verificationGasLimit:
        150000n + (isDeployed ? 0n : await this.creationGas()), // default verification gas ?+ create2 cost
  ...
  public calldata = (action: ArrayLike<Action>, version?: string) => {
    if (!action || action.length == 0)
      throw new Error("action can't be empty!");
    ...
    if (action.length > 1) {
      ...
      switch (version) {
        case VERSION.v6:
          ...
          return (
            EXECUTE_BATCH.v6.sig +
            this.abiCoder
              .encode(EXECUTE_BATCH.v6.type, [dest, value, func])
              .substring(2)
          );
        case VERSION.v5:
          return (
            EXECUTE_BATCH.v5.sig +
            this.abiCoder
              .encode(EXECUTE_BATCH.v5.type, [dest, func])
              .substring(2)
          );
        default:
    ...
    return this.aaAccount.interface.encodeFunctionData("execute", [
      dest,
      value ?? 0n,
      func,
    ]);
  ...
  public gasInfo = async () => {
    ...
    if (!gasPrice)
      throw new Error("The Gas-rpc-method(eth_gasPrice) is unsuitable");
    return {
  ...
  public creationGas = async () => {
    return await this.signer!.provider!.estimateGas({
      to: this.aaAccountFactory.target,
      data: this.creationCode,
    });
  };
  ...
  protected safeRequestBigInt = async (method: string) => {
    try {
      return await (this.signer!.provider! as JsonRpcApiProvider)
        .send(method, [])
        .then(BigInt);
    } catch {
      return undefined;
