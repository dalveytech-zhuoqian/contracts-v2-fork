...
async function encodeData(account: string, market: string, isLong: boolean) {
  // 创建一个新的合约对象
  const MarketReader = new ethers.Contract(
  ...
  calldata[0] = abicode.encode(
    ...
    [
      addressJSON.MarketReader,
      MarketReader.interface.encodeFunctionData("getFundingFee", [
        account,
        market,
        isLong,
      ]),
  ...
  calldata[1] = abicode.encode(
    ...
    [
      addressJSON.MarketReader,
      MarketReader.interface.encodeFunctionData("getMarket", [market]),
    ],
  );
  ...
  calldata[2] = abicode.encode(
    ...
    [
      addressJSON.MarketReader,
      MarketReader.interface.encodeFunctionData("getPositions", [
        account,
        market,
      ]),
    ],
