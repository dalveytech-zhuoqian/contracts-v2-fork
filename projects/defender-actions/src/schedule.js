...
exports.handler = async function (credentials) {
  ...
  const when = (await provider.getBlock('latest')).timestamp + 60 * 10;
  ...
  calldata[1] = AccessManager.interface.encodeFunctionData('schedule', [
    addressJSON.rewardDistributor,
    intervalData,
    when,
  ]);
  const tx = await AccessManager.multicall(calldata, { gasLimit: '200000' });
  return tx;
};
