...
const addressJSON = {
  ...
  accessManager: '0xEE3B9EE572b4e95c5bb02B4C2438b58620f78A31',
  caller: '0x3E50Fadf9943282d34F48ACF3E559e690C3022eE',
};

async function fetchAPRData() {
  // let chainId = getChainId();
  let response = await axios.get(`https://mumbai-info.blex.io/api/infomation/getapr?ChainId=4002`);
...
async function generateSetAPRdata() {
  ...
  const apr = ethers.utils.parseUnits(data['apr'], 18).div(10 ** 10);
...
async function generateSetTokensPerIntervalData() {
  const data = await fetchAPRData();
  const tokensPerInterval = ethers.utils.parseUnits(data['tokensPerInterval'], 18).div(10 ** 12);
...
exports.handler = async function (credentials) {
  ...
  const AccessManager = new ethers.Contract(addressJSON.accessManager, AccessManagerABI, signer);
  let calldata = new Array(2);
  const signerAddress = await signer.getAddress();
  calldata[0] = AccessManager.interface.encodeFunctionData('execute', [
    signerAddress,
    addressJSON.vaultReward,
    aprData,
  ]);
  calldata[1] = AccessManager.interface.encodeFunctionData('execute', [
    signerAddress,
    addressJSON.rewardDistributor,
    intervalData,
  ]);

  return AccessManager.multicall(calldata, { gasLimit: '200000' });
};
