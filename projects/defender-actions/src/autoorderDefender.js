...
function blexMutiCallContract(signer) {
  const contract = new ethers.Contract(addressJSON.BlexMutiCall, BlexMutiCallABI, signer);
  return contract;
}
...
function encodeOrderPerformUpkeepData(checkData) {
  const autoOrder = new ethers.Contract(addressJSON.AutoOrderV2, autoOrderV2ABI);
  return ethers.utils.defaultAbiCoder.encode(
    ['address', 'bytes'],
    [addressJSON.AutoOrderV2, autoOrder.interface.encodeFunctionData('performUpkeep', [checkData])],
  );
}
...
function encodeLiqPerformUpkeepData(checkData) {
  ...
  return ethers.utils.defaultAbiCoder.encode(
    ['address', 'bytes'],
    [addressJSON.AutoLiq, autoOrder.interface.encodeFunctionData('performUpkeep', [checkData])],
  );
}
    ...
    for (let isIncrease = 0; isIncrease < 2; isIncrease++) {
      ...
      checkDatas.push(
        ethers.utils.defaultAbiCoder.encode(
          ['address', 'bytes'],
          [addressJSON.AutoOrderV2, contract.interface.encodeFunctionData('checkUpkeep', [data])],
        ),
      );
    }
  }
...
function encodeLiqCheckData(contract, marketAddress) {
  const checkData = ethers.utils.defaultAbiCoder.encode(['address'], [marketAddress]);
  return [
    ethers.utils.defaultAbiCoder.encode(
      ['address', 'bytes'],
      [addressJSON.AutoLiq, contract.interface.encodeFunctionData('checkUpkeep', [checkData])],
    ),
  ];
}
...
async function callCheckUpKeep(contract, orderDatas, liqDatas) {
  ...
  try {
    ...
    for (let index = 0; index < results.length; index++) {
      response.push(ethers.utils.defaultAbiCoder.decode(['bool', 'bytes'], results[index]));
    }
    return response;
  } catch (error) {
...
exports.handler = async function (credentials) {
  ...
  const performDatas = await callCheckUpKeep(blexMutiCall, checkOrderDatas, checkLiqDatas);
  const executeArrs = [];
