...
async function readMarketABI() {
    let response = await axios.get(
        `${HOST}/arbitrum_42161/artifacts/contracts/market/Market.sol/Market.json`
    );
...
async function readAddress(chainId, signer) {
    ...
    let commonContract = await axios.get(
        `${HOST}/${CHAIN_ID_MAP[chainId]}/${CHAIN_ID_MAP[chainId]}.json`
    );

    let marketReaderABI = await readMarketReaderABI();
    let marketReader = new ethers.Contract(
        commonContract.data.MarketReader,
        marketReaderABI,
        signer
    );
    ...
    for (let index = 0; index < allMarket.length; index++) {
        ...
        let fantomMarket = await axios.get(
            `${HOST}/${CHAIN_ID_MAP[chainId]}/${CHAIN_ID_MAP[chainId]}-${marketName}.json`
        );
...
exports.handler = async function (credentials) {
    ...
    for (let index = 0; index < addressBook.Markets.length; index++) {
        let market = new ethers.Contract(
            addressBook.Markets[index],
            MarketABI,
            signer
        );
        console.log(addressBook.MarketVailds[index]);
        console.log(addressBook.PositionSubMgr);
        await waitTx(market.setMarketValid(addressBook.MarketVailds[index], {
            gasLimit: "2000000"
        }))

        await waitTx(market.setPositionMgr(addressBook.PositionSubMgr, false, {
            gasLimit: "2000000"
        }));
