...
async function readMarketRouterABI() {
  let response = await axios.get(`https://raw.githubusercontent.com/blex-dex/address/arbitrum_42161/MarketRouter.json`);
  return response.data.abi;
}
...
exports.handler = async function (credentials) {
  ...
  for (let index = 0; index < addressBook.Market.length; index++) {
    let market = new ethers.Contract(addressBook.Market[index], MarketABI, signer);
