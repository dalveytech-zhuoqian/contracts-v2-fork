...
async function setAccessManagerAddress(
  ...
  const accessManagerAddress = (await deployments.get("BlexAccessManager"))
    .address;
  const marketDiamond =
    await ethers.getContract<MarketDiamond>("MarketDiamond");
  const deployerDeployments = await setupUser(deployer, {
    MarketDiamond: marketDiamond,
  });
  const authority = await deployerDeployments.MarketDiamond.authority();
  if (authority === ethers.ZeroAddress) {
    await waitFor(
      deployerDeployments.MarketDiamond.setAuthority(accessManagerAddress),
    );
