...
async function setAccessManagerAddress(
  hre: HardhatRuntimeEnvironment,
): Promise<void> {
  const { deployments, getNamedAccounts } = hre;
  const { deployer } = await getNamedAccounts();

  const DummyDiamondImplementation = (
    await deployments.get("DummyDiamondImplementation")
  ).address;
  const DiamondEtherscanFacet =
    await ethers.getContract<DiamondEtherscanFacet>("MarketDiamond");
  const deployerDeployments = await setupUser(deployer, {
    MarketDiamond: DiamondEtherscanFacet,
  });
  const currentImplementation =
    await deployerDeployments.MarketDiamond.implementation();
  if (currentImplementation !== DummyDiamondImplementation) {
    if (ZeroAddress === currentImplementation) {
      console.log(
        "current implementation is zero address, setting new implementation",
      );
    } else {
      console.log("implementations don't match, setting new implementation");
    }
    await waitFor(
      deployerDeployments.MarketDiamond.setDummyImplementation(
        DummyDiamondImplementation,
      ),
    );
  } else {
