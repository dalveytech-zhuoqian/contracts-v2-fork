...
library PositionSubMgrLib {
    ...
    ) internal pure returns (int256 withdrawFromFeeVault, int256[] memory afterFees) {
    ...
    function calculateNewCollateral(
        ...
        if (_params.collateralDelta.toInt256() + dPNL - fees < 0)
            return (_position.collateral.toInt256() + dPNL - fees).toUint256(); // 如果保证金增量加上盈亏减去费用小于0，则返回保证金加上盈亏减去费用的值
    ...
    function calculateFundFeeLoss(
        ...
        int256 fFee = fs[uint8(FeeRouterLib.FeeType.FundFee)]; // 资金费用
        int256 remain = -fs[uint8(FeeRouterLib.FeeType.CloseFee)] + coll + pnl; // 剩余资金
        if (fFee > 0 && fFee > remain)
            return uint256(fFee - SignedMath.max(remain, 0)); // 计算资金费用亏损
    }
